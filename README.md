...
1. **Поиск в реальном времени**
   - Функциональное поле поиска в `SearchAppBar` с onChange handler
   - Debouncing с задержкой 400ms через custom hook `useDebounce`
   - Предотвращение избыточных API запросов при печати
   - Кнопка очистки поиска с иконкой "X"

2. **Фильтрация по категориям**
   - Все категории кликабельны в `Categories` компоненте
   - Визуальное выделение активной категории (selected state, bold text)
   - Опция "Все товары" для сброса фильтра категории
   - Toggle behavior - повторный клик снимает фильтр

3. **Комбинирование фильтров**
   - Одновременная работа поиска + категория
   - API запросы с параметрами: `/products?q=laptop&category=Gaming`
   - Фильтры синхронизированы через React Context (`FilterContext`)
   - State sharing между SearchAppBar, Categories и Products

4. **Debouncing для оптимизации**
   - Custom hook `useDebounce` в `src/hooks/useDebounce.ts`
   - Задержка 400ms перед отправкой запроса
   - Значительное снижение нагрузки на API при быстрой печати
   - Очистка таймеров при размонтировании

5. **URL синхронизация**
   - Фильтры сохраняются в URL query parameters
   - Формат: `?q=search&category=Laptops&sort=price-asc`
   - Возможность поделиться ссылкой с активными фильтрами
   - Восстановление фильтров при загрузке страницы из URL
   - Использован `window.history.replaceState` для обновления без перезагрузки

6. **Кнопка "Очистить фильтры"**
   - Кнопка в боковой панели Categories
   - Кнопка-иконка в SearchAppBar (FilterList icon)
   - Сбрасывает все фильтры: поиск, категорию, сортировку
   - Отображается только когда есть активные фильтры

7. **Визуальные индикаторы активных фильтров**
   - Chips под панелью навигации с текущими фильтрами
   - Каждый chip можно закрыть отдельно
   - Динамическое отображение: "Поиск: ...", "Категория: ...", "Сортировка: ..."
   - Styled с полупрозрачным белым фоном

**Бонусные функции:**

8. **Сортировка товаров**
   - Dropdown в SearchAppBar с опциями:
     - Название (А-Я / Я-А)
     - Цена (по возрастанию / убыванию)
   - Клиентская сортировка через `useMemo` для производительности
   - Сортировка применяется после загрузки и фильтрации

9. **Счетчик результатов**
   - Отображение "Найдено товаров: X" над списком
   - Показывает total count из API response
   - Обновляется при изменении фильтров
   - Дополнительная информация о текущей сортировке

**Технические решения:**

- **State Management**: React Context API (`FilterContext.tsx`) для глобального состояния фильтров
- **Performance**: 
  - `useMemo` для мемоизации отсортированных товаров
  - `useCallback` для стабильных ссылок функций
  - Debouncing для снижения количества API запросов
- **Architecture**: Четкое разделение ответственности:
  - `FilterContext` - управление состоянием
  - `useDebounce` - reusable hook для debouncing
  - Компоненты отвечают только за UI
- **TypeScript**: Строгая типизация `FilterState` и context типов
- **URL Sync**: Bidirectional синхронизация state ↔ URL
- **UX**: 
  - Мгновенная обратная связь при изменении фильтров
  - Прокрутка наверх при новом поиске
  - Полезные подсказки в пустых состояниях

**Тестирование:**

1. Откройте http://localhost:5173
2. Введите поиск в header - результаты обновятся с задержкой 400ms
3. Выберите категорию - фильтрация применится мгновенно
4. Скомбинируйте поиск + категорию - оба фильтра работают вместе
5. Измените сортировку - товары переупорядочатся
6. Проверьте URL - должен содержать параметры фильтров
7. Скопируйте URL и откройте в новой вкладке - фильтры восстановятся
8. Нажмите "Сбросить фильтры" - все сбросится

**Допущения:**

- Debounce delay 400ms оптимален для баланса UX/производительность
- Сортировка реализована на клиенте, т.к. API не поддерживает параметр `sort`
- URL синхронизация использует `replaceState` (не `pushState`) чтобы не засорять history
- Фильтры сбрасывают пагинацию (возврат к странице 0)
